...
%As expected, biomass densities in the temperate ecosystem followed seasonal cycles, evident in Figure~\ref{fig:chap:res:ts:expno1}.
% Annual turn-over reached up to one order of magnitude for autotrophs, while less, but pronounced, variation was found for herbivores.
%  Omnivores and carnivores closely followed these cycles  at lower biomass densities (respective order) and with smaller amplitudes (cf. Figure~\add{TS APPENDIX}). \\\\
%  The aseasonal system was generally characterized by more stable biomass densities over time, while variation resulting from random organismal interactions encoded in the model, as opposed seasonal dynamics, dominates. 
...
Biomass densities in the temperate ecosystem followed seasonal cycles with large annual turnover, as indicated by the confidence intervals in Figure~\ref{fig:chap:res:dyn:comp} (cf. Figure~\add{APPENDIX TS} for detailed time series plots). In contrast, the biomass densities in the aseasonal system were more stable (i.e. no seasonal fluctuations, as expected) and had more random variation, likely related to trophic and population dynamics (cf. Figure~\add{APPENDIX TS}).
