...
*Note:  'experiments' should be termed 'scenarios'; will be changed.*
...
```{r summary, echo=FALSE, results='asis', cache=TRUE}
...
The following set of plots show the logged median biomass density [$kg/km^{2}$] for the aseasonal environment in Cell0 (**left**) and the seasonal environment in Cell1 (**right**) for experiment 1 through 4 (i.e. unaltered system, and one carnivorours group removed for the remainder). The y-axes are scaled identically for easy comparison.   

After an initial 'burn-in' phase of approx. 200 to 400 time steps (15 to 30 years), the respective systems tend to develop towards a dynamic equilibrium. However, especially for **autotrophs and herbivores in the aseasonal**, and for **herbivores and carnivores in the seasonal** system a **slight increase over time** can be seen, calling for additional measures / analysis (e.g time series analysis). The **aseasonal** environment seems to be **robust to alterations** of both functional diversity and composition, as both biomass density and ratios appear constant, **except for the case of removing iteroparous ectotherms (C-ect-i)**, where variability increases and absolute maximum values are lowered. 

For the **aseasonal** environment, the **over-all patterns remain surprisingly constant** (neglegting a slightly increased carnivore : herbivore ratio when endotherm. carnivores are removed) for both **experiment  1 through 3**. The removal of iteroparous ectotherm. **carnivores (C-ect-i; experiment 4)** again has a dramatic **impact on both ratios and absolute values for all trophic groups**, with a pronounced increase of herbivores and omnivores, and a stark decrease for carnivores.
...
data_plot(stats, expno = 1:4, logscale = T)



```

The following two plots compare the logged biomass density of by trophic group for each experiment in the aseasonal (**top**) and seasonal (**bottom**) environment, for years *50 to 100*. Year 50 was chosen as a preliminary cut-off to insepct the dynamics of the system further. This approach revealse clear differences between for individual trophic groups for the depicted scenarios (1 to 4). The removal of endotherm (**E2**) and ectotherm. iteroparous carnivores (**E4**) show the greatest deviation from the unaltered system.

```{r compare, echo=FALSE, fig.height=12, fig.width=12, cache=FALSE}

data_boxplot(stats, expno=c(1:4), logscale = T, cutoff=500)
