...
```{r OverviewPlot, echo=F, fig.height=16, fig.width=12}



invisible(data_plot(statsFull, expno = 1:4, logscale = T))
...
invisible(data_boxplot(statsFull, expno=c(1:4), logscale = T, cutoff=500))
...
Linear models were fitted to each decomposed time series to test for non-zero slopes (i.e. true trends). An optimaziation function was implemented to identify the optimal time interval (i.e. cut-off) by minimizing the number of non-zero slopes found throughout all experiments and functional groups. A scanning window of 50 to 95 years as cut-off was used.

```{r tsdecompID, echo=F, fig.height=6, fig.width=12}
